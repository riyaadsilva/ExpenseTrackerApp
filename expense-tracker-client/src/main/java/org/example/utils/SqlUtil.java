...
public class SqlUtil {
    ...
    public static List<TransactionCategory> getAllTransactionCategoriesByUser(User user){
        ...
        try{
            conn = ApiUtil.fetchApi(
                    "/api/v1/transaction-categories/user/" + user.getId(),
                    ApiUtil.RequestMethod.GET, null
            );
    ...
    public static List<Transaction> getRecentTransactionByUserId(int userId, int startPage, int endPage, int size){
        ...
        try{
            ...
            final JsonArray resultJsonArray = new JsonParser().parse(results).getAsJsonArray();
    ...
    try {
        ...
        if (status != HttpURLConnection.HTTP_OK && status != HttpURLConnection.HTTP_CREATED) {
            String errorResponse = ApiUtil.readApiResponse(conn);
            System.out.println("Error(postCreateUser): Status " + status + " | " + errorResponse);
            return false;
        }

        // Optional: Log success message
        String successResponse = ApiUtil.readApiResponse(conn);
        System.out.println("User created successfully: " + successResponse);
